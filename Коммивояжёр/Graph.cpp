...
public:
	...
	void setVertex(DVertex& vertex1, DVertex& vertex2, string text)
	{
		...
		//Проходим по условиям положения x y координат относительно нуля для поворота ребра в идеальное положение(центр окружностей) между двух вершин
...
{
	int min = INT_MAX;
	int rowmin[7][1] = {};//Массив для поиска максимального значения по строкам
	//Ищем минимум по строкам используя циклы 
	for (int i = 0; i < V1; i++)
	{
		min = INT_MAX;
		for (int j = 0; j < V2; j++)
	...
	for (int i = 0; i < V1; i++)
	{
		for (int j = 0; j < V2; j++)
		{
			if (a[i][j].num != -1 && a[i][j].num != 0) a[i][j].num = a[i][j].num - rowmin[i][0]; //Если путь существует и он !=0, то вычитаем минимальное значение из строки
		}
		
	}
	min = INT_MAX;//Возвращение минимума для дальнейшей работы
	int colmin[1][7] = {};//Массив для поиска минимального значения по строкам
	for (int j = 0; j < V2; j++)//Проходим весь граф 
	{
		min = INT_MAX;
		for (int i = 0; i < V1; i++)
		{
			if (a[i][j].num < min && a[i][j].num != -1 && i != j)	min = a[i][j].num; //Присавиваем min, если нашли меньше
		}
	...
	for (int j = 0; j < V2; j++) //Проходим весь граф
	{
		for (int i = 0; i < V1; i++)
		{
			if (a[i][j].num != -1 && a[i][j].num != 0) a[i][j].num = a[i][j].num - colmin[0][j]; //Если путь существует и он !=0, то вычитаем минимальное значение из строки
		}
	}

	min = INT_MAX;//Возвращение минимума для дальнейшей работы
	int sum = 0;
	for (int i = 0; i < 7; i++)//Проходим весь граф
	{
		for (int j = 0; j < 7; j++)
		{
			...
			if (a[i][j].num == 0 && i != j)
			{
				...
				sum += min;//Складываем оценку нуля по строкам (ищем минимальный элемент в строке, где находится ноль)
				min = INT_MAX;//Возвращение минимума для дальнейшей работы
				for (int k = 0; k < 7; k++)
				{
				...
				sum += min;//Сложим оценку из мин элемента по строке и столбцу
				a[i][j].rate = sum; //Записываем оценку (rate)
	...
	way ways1[7];//Создаем массив ways для отслеживания городов, в которые мы идём 

	for (int k = 0; k < 4; k++)
	{
		int max = INT_MIN;//Переходим к макс
		//Объявляем переменные в которые будут записываться номер строки и столбца в которых находится ноль с наибольшей оценкой
		int max_col = 0;
		int max_row = 0;
		for (int i = 0; i < V1; i++)//Проходим весь граф
		{
			for (int j = 0; j < V2; j++)
			{
				if (a[i][j].rate > max && j != i) //Находим максимум вне диалонали
				{
					max = a[i][j].rate;
					max_row = i;
					max_col = j;
		...
		//При смене местами городов ставим -1 
		a[max_col][max_row].num = -1;
		a[max_col][max_row].rate = 0;

		ways1[k].first = max_row + 1;//Записываем в массив с путями города
		ways1[k].second = max_col + 1;
		result += b[max_row][max_col];//Добавляем в ребультат
	}
	...
	for (int i = 0; i < V1; i++)//Проходим весь граф
	{
		for (int j = 0; j < V2; j++)
		{
			if (a[i][j].num > 1) //Оценочный проход
			{
				row_p = i;
				col_p = j;
		...
		//Получение кратчайшего не сортированного пути
	}
	int tmp_help = 1;
	...
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 7; j++)
		{
			if (ways1[j].first == tmp_help)	//Расстановка пути по порядку
			{
...
int main()
{
	...
	while (window.isOpen())
	{
		Vector2i mouse_pos = Mouse::getPosition(window); //Вектор целого типа
		Event event;
